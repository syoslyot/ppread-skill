...
上表沒有 `partial` 這一列，卻把它當成前提（「目標模式為 `partial`」預設模式已經選出）。
`broad: partial, deep: absent`、`broad: read, deep: partial`、兩者皆 `partial`
這三種在不帶 flag 時可實際發生的狀態，上表因此答不出動作——而這正是 `.part.md`
存在的理由：一次中斷的執行，之後被裸的 `/ppread <source>`接續。

以下規則涵蓋全部十六種 `broad`／`deep` 狀態組合，與上表在**已定義**的格子上結果
一致；兩者衝突時，以本節規則為準：

1. 一個模式的狀態為 `unread` 或 `read` 時視為**已完成**；`absent`、`partial` 皆非已完成。
2. **不帶 flag**：依序看 broad、deep，取第一個未完成的模式為待處理模式；兩者皆已完成則停止並回報兩份檔案的路徑。
3. **帶 `--broad` 或 `--deep`**：待處理模式就是該旗標指定的模式。
4. 決定待處理模式後：`absent` → 寫入；`partial` → 詢問續寫或重寫（見上方既有的續寫／重寫說明）；已完成 → 回報該檔案路徑並停止。

上表可以保留作為速查，但決策依據是本節的規則，不是上表本身。
